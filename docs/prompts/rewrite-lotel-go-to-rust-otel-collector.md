...
### Project Description
Rewrite lotel from Go to Rust as an importable OpenTelemetry collector utility. Implement a native Rust OTel collector that supports all basic features of the OTel collector Docker container, eliminating the Docker dependency. The library should be importable by other Rust projects (e.g., multisamples at `$HOME/git/multisamples`) to programmatically start and stop the collector server. Additionally, provide a lotel CLI (rewritten in Rust) that uses this library, maintaining full feature parity with the existing Go CLI.

### Technical Stack
Rust and OpenTelemetry

### Specific Requirements
Support the feature set of https://github.com/open-telemetry/opentelemetry-collector. Because we may have to update and maintain parity over time, we should track somewhere which commit in https://github.com/open-telemetry/opentelemetry-collector was used for determining current parity. The upstream repo will be checked out at `$HOME/git/opentelemetry-collector` on any machine doing feature work on lotel.
